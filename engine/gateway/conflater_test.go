...
func TestConflater(t *testing.T) {
  conflator := Conflator{Options: ConflationOptions { OneToMany:true } }
  conflator.SubTasks = make(map[int][]SubTask)
  conflator.Issues = make(map[int]github.Issue)
  ...
  title := "Issue Title"
  gitIssue := github.Issue { Number: &number,  Title: &title, Body: &issue}
  ...
  gitPull := github.PullRequest { Number: &pullNumber, Title: &pullTitle, Body: &pull}
