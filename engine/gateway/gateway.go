...
func (c *Gateway) GetPullRequests() ([]*github.PullRequest, error) {
	...
	for {
		pullRequests, resp, err := c.Client.PullRequests.List(variables[0], variables[1], pullsOpt)
		if err != nil {
			return nil, err
		}
...
func (c *Gateway) GetIssues() ([]*github.Issue, error) {
	...
	for {
		issues, resp, err := c.Client.Issues.ListByRepo(variables[0], variables[1], issuesOpt)
